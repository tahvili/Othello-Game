...
public class RandomHint extends Button implements Observer {
	...
	@Override
	/**
	 * A method that updates the View based on what the RandomHint is
	 * 
	 */
	public void update(Observable o) {
		...
		if (model.getHint() == "Random" && model.RandomAlreadyMade) {
				...
				if (source instanceof ButtonLabel) {
					...
					if (!model.othello.othello.isGameOver() || model.getLostTime()==' ') {
						if (GridPane.getColumnIndex(source) == hintMove.getCol()
							...
							btn.setId("hint");
							this.random = source;
							model.RandomAlreadyMade = false;
							model.GreedyAlreadyMade = true;
						}
					}
				}

				if (source instanceof Button) { // disabling other hint buttons when RandomHint button has been
												// activated
			...
			// changing the color of the RandomHint button when selected
			this.setStyle(
					"-fx-background-color: #9000b9; -fx-border-color: #9000b9; -fx-border-width: 0px; -fx-font-weight:bold; -fx-font-size:11px;");

		}

		if (model.othello.othello.isGameOver() || model.getHint() == "" || model.getHint() == "Greedy") {
			// changing the color of the RandomHint button back to original
			this.setStyle(
		...
		if (model.othello.othello.isGameOver() || model.getLostTime()!=' '||model.getHint() == "") {
			if (this.random != null) { // changing the highlighted hint color back to normal available move color
				model.RandomAlreadyMade = true;
				ButtonLabel btn = (ButtonLabel) this.random;
			...
			// re-enabling other hint buttons when RandomHint button is no longer activated
			if (this.single != null) {
