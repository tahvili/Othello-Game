...
public class Othello extends Observable implements Visitable {
	...
	public char getToken(int row, int col) {
		TokenVisitor tv = new TokenVisitor(row, col);
		return (char) accept(tv);
	}
	...
	public boolean move(int row, int col) {
		...
		MoveVisitor mv = new MoveVisitor(row, col);

		if ((boolean) accept(mv)) {
	...
	public int getCount(char player) {
		CountVisitor cv = new CountVisitor(player);
		return (int) accept(cv);
	}
	...
	public Othello copy() {
		...
		Othello currentboard = (Othello) accept(cv);
		currentboard.numMoves = this.numMoves;
		currentboard.whosTurn = this.whosTurn;
		return currentboard;
	}
	...
	public String getBoardString() {
		BoardStringVisitor bv = new BoardStringVisitor();
		return (String) accept(bv);
	}

	@Override
	/**
	 * Method to accept the visitor visiting this class
	 * 
	 */
	...
	public static void main(String[] args) {
		...
		CountVisitor v = new CountVisitor(OthelloBoard.P1);
		TokenVisitor vt = new TokenVisitor(3, 4);
		CopyVisitor cv = new CopyVisitor();
		...
		while (!o.isGameOver()) {
			...
			if (o.move(row, col)) {
				...
				System.out.println(o.accept(v));
			}
		}
		System.out.println(o.accept(vt));
		System.out.println("Our Copy:" + ((Othello) o.accept(cv)).getBoardString());
	}
